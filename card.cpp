...
std::pair<int32, int32> card::get_atk_def() {
	if (assume_type == ASSUME_ATTACK)
		return std::pair<int32, int32>(assume_value, 0);
	...
	temp.attack = batk;
	temp.defense = bdef;
	std::pair<int32, int32> base_val = get_base_atk_def();
	atk = base_val.first;
	def = base_val.second;
	temp.attack = atk;
	temp.defense = def;
