...
//millux
uint32_t card::get_ritual_type() {
	if(current.location == LOCATION_SZONE && (data.type & TYPE_MONSTER))
		return data.type;
	return get_type();
}
uint32_t card::set_entity_code(uint32_t entity_code, bool remove_alias) {
	card_data dat;
...
int32_t card::is_setable_szone(uint8_t playerid, uint8_t ignore_fd, uint8_t toplayer, uint32_t zone) {
	uint32_t tp = current.controler;
	if(toplayer < 2)
		tp = toplayer;
...
int32_t card::is_attack_decreasable_as_cost(uint8_t playerid, int32_t val) {
...
int32_t card::is_defense_decreasable_as_cost(uint8_t playerid, int32_t val) {
	if(!(data.type & TYPE_MONSTER) && !(get_type() & TYPE_MONSTER))
		return FALSE;
