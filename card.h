...
public:
	...
	uint32_t get_ritual_type();
	uint32_t set_entity_code(uint32_t entity_code, bool remove_alias = false);
	uint32_t get_summon_info();
	...
	int32_t is_setable_szone(uint8_t playerid, uint8_t ignore_fd = 0, uint8_t toplayer = 2, uint32_t zone = 0xff);
	...
	int32_t is_attack_decreasable_as_cost(uint8_t playerid, int32_t val);
	int32_t is_defense_decreasable_as_cost(uint8_t playerid, int32_t val);
