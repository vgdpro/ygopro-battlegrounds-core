...
//2pick
int32_t scriptlib::duel_save_pick_deck(lua_State * L) {
	check_param_count(L, 2);
	check_param(L, PARAM_TYPE_GROUP, 2);
	int32_t playerid = lua_tointeger(L, 1);
...
int32_t scriptlib::duel_is_player_need_to_pick_deck(lua_State * L) {
...
int32_t scriptlib::duel_get_start_count(lua_State * L) {
	check_param_count(L, 1);
	int32_t p = lua_tointeger(L, 1);
	if(p != 0 && p != 1)
		luaL_error(L, "Parameter 1 should be 0 or 1.", 2);
...
int32_t scriptlib::duel_get_disable_field(lua_State * L){
	duel* pduel = interpreter::get_duel_info(L);
...
int32_t scriptlib::duel_announce_card(lua_State * L) {
	...
	if(lua_gettop(L) == 1) {
		...
		pduel->game_field->core.select_options.push_back((uint32_t)lua_tointeger(L, 2));
...
int32_t scriptlib::duel_announce_card_silent(lua_State * L) {
	duel* pduel = interpreter::get_duel_info(L);
